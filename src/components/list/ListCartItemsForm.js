	...
	return (
		<div className='position-relative'>
			...
			{items.map((item, index) => (
				<div
					key={index}
					className={`d-flex align-items-center res-flex-column item${index === items.length - 1 ? ' last-item' : ''
						}`}
				>
					<div
						style={{
			...
			{items.reduce(
				...
				true
					...
					<div className='d-flex flex-wrap justify-content-end align-items-center mt-4 pt-1 border-top border-primary res-flex-justify-between'>
						...
						<button
							className={`btn rounded-1 ${showCheckoutFlag ? 'btn-primary' : 'btn-outline-primary'
								} ripple`}
							type='button'
